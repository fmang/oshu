...
void oshu::scale_view(struct oshu::view *view, double factor)
{
	view->zoom *= factor;
	view->size /= factor;
}

void oshu::resize_view(struct oshu::view *view, oshu::size size)
{
	view->origin += view->zoom * (view->size - size) / 2.;
	view->size = size;
}

void oshu::fit_view(struct oshu::view *view, oshu::size size)
{
...
oshu::point oshu::project(struct oshu::view *view, oshu::point p)
{
	return p * view->zoom + view->origin;
}

oshu::point oshu::unproject(struct oshu::view *view, oshu::point p)
{
	return (p - view->origin) / view->zoom;
}

void oshu::reset_view(struct oshu::display *display)
{
	int w, h;
